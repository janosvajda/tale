...
[`tale.project.json`](../project/tale.project.json) is the first editor project, manually
...
| `deploymentDirectory` | Optional directory suggestion saved by New project. It may not exist yet. Deployment asks the user to choose a directory again; this field grants no write permission and is not emitted in Tale output. |
...
| `exports` | Legacy JSON field retained for compatibility. At most one definition is compiled. With an empty list and exactly one TALE root, the file is automatically `.tale/project.tale`. Multiple definitions remain editable but require choosing one Tale in Project settings before deployment. Environments are listed in agent instructions, not separate files. |
...
Deploy compiles the selected Tale (or the only Tale root), writes one generated
`.tale` file into the destination's `.tale/` directory, and updates selected agent
instructions to load that file and list the project's environments. The deployed bytes match Preview Tale.
